...
It seems that {shiny} doesn't show the content of a different route if the structure
of the `element` argument is identical in both routes. In case the `element` structure is similar,
you can refresh the session each time the user click on a router link by observing
the `NavLink.shinyInput()` input in the session.
