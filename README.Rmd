...
[![CRAN status](https://www.r-pkg.org/badges/version/shinyReactRouter)](https://CRAN.R-project.org/package=shinyReactRouter)
...
The goal of **shinyReactRouter** is to provide a wrapper around [React Router (v6)](https://reactrouter.com/6.30.0).
...
In order to render objects from the server, you should observe user clicks on
each page produced by `NavLink.shinyInput()` (learn more about **shiny.react** `*.shinyInput` 
wrappers [here](https://appsilon.github.io/shiny.react/articles/shiny-react.html#creating-input-wrappers)). 

It is **strongly** recommended to reload the session using the `observeEvent({})`
function to ensure server objects renders properly when users clicks on a new page.
...
For simple apps, wrapping the server R code in `observe({})` also works. But for
more advanced apps it is recommended to reload the session.
...
Thanks to the client side routing provided by React Router, you can also create multiple pages in your Quarto and Rmarkdown documents (as no server is required).
...
shinyReactRouter implements React Router [v.6.30.0](https://reactrouter.com/6.30.0).
