...
`r badger::badge_custom(names(dependencies)[1], dependencies[1], "blue", "https://mui.com/material-ui/getting-started/")`
...
It seems that {shiny} doesn’t show the content of a different route if the structure of the `element` argument is identical in both routes. This is an issue if multiple pages share the same HTML structure. More examples of this issue are given [here](https://github.com/lgnbhl/reactRouter/issues/1).

A possible workaround, in case the element argument structure is similar, is to refresh the session each time the user click on a router link by observing the `NavLink.shinyInput()` input in the session.
