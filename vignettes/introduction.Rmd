...
`createMemoryRouter()` vs. `createBrowserRouter()`) and when to set
`reloadDocument = TRUE`, see `vignette("routers", package = "reactRouter")`.
...
If your dynamic page renders **server-rendered** Shiny output (`uiOutput()`,
`renderUI()`, `plotOutput()`), set `reloadDocument = TRUE` on the relevant
links so the Shiny session re-initialises with the new URL hash. See
`vignette("routers", package = "reactRouter")` for the full reference.
